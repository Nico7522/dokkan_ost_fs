...
-- Récupère toutes les nouvelles carte en version SSR (toutes les nouvelles sorties)
SELECT cards.name, card_awakening_routes.open_at FROM card_awakening_routes
JOIN cards ON card_awakening_routes.card_id = cards.id
WHERE type = 'CardAwakeningRoute::Zet'
AND substr(card_awakening_routes.open_at,0, 5) < '2026'
ORDER BY card_awakening_routes.open_at DESC
